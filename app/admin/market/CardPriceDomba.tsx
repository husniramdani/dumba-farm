...
export default function CardPriceDomba() {
  const { data, isLoading, error, refetch } = useLastMarketPrice('DOMBA')
  const [isScraping, setIsScraping] = useState(false)

  const handleScrapeAndRefetch = async () => {
    setIsScraping(true)
    try {
      await axios.get('/api/domba')
      await refetch()
      toast.success('Berhasil memperbarui harga domba')
    } catch (err) {
      console.error('Error scraping market price:', err)
      toast.error('Gagal memperbarui harga domba')
    } finally {
      setIsScraping(false)
    }
  }
  ...
  return (
    <Card>
      <CardHeader className="relative">
        <CardTitle className="font-medium">Harga Domba</CardTitle>
        <Button
          size="sm"
      ...
      <CardFooter>
        <Button asChild size="sm" className="w-full">
          <Link href="/admin/market/domba">Lihat History</Link>
        </Button>
      </CardFooter>
    </Card>
  )
