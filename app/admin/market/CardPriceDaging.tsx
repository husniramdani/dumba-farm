...
export default function CardPriceDaging() {
  const { data, isLoading, error, refetch } = useLastMarketPrice('DAGING')
  ...
  return (
    <Card>
      <CardHeader className="relative">
        <CardTitle className="font-medium">Harga Daging</CardTitle>
        <Button
          size="sm"
        ...
        {isLoading ? (
          ...
          <>
            <div className="text-2xl font-bold">
              Rp {data?.price?.toLocaleString('id-ID') || 0} /kg
            </div>
      ...
      <CardFooter>
        <Button asChild size="sm" className="w-full">
          <Link href="/admin/market/daging">Lihat History</Link>
        </Button>
      </CardFooter>
    </Card>
  )
